const shipSchema = require("../../schemas/ship-schema.js");
    ...
    if (user) {
      if (user.id === message.author.id) {
        const embed = new Discord.MessageEmbed()
          ...
          .setDescription("❌ | We know that you love yourself!");
        message.channel.send(embed);
        return;
      }

      const result = await shipSchema.findOne({
      ...
      if (result) {
        ...
        const background1 = await Canvas.loadImage("./images/ship.jpg");
        ctx.drawImage(background1, 0, 0, canvas.width, canvas.height);

        const heart1 = await Canvas.loadImage("./images/heart.png");
        ctx.drawImage(heart1, 263, 35, 175, 175);
        ...
        ctx.fillText(result.love, canvas.width / 2.0, canvas.height / 1.8);
      ...
      const background = await Canvas.loadImage("./images/ship.jpg");
      ctx.drawImage(background, 0, 0, canvas.width, canvas.height);

      const heart = await Canvas.loadImage("./images/heart.png");
      ctx.drawImage(heart, 263, 35, 175, 175);
      ...
      ctx.font = "55px sans-serif";
      ctx.fillStyle = "#ffffff";
      ctx.fillText(randomMessage, canvas.width / 2.0, canvas.height / 1.8);
      ...
      await shipSchema.findOneAndUpdate(
        {
          userId: userAuthor.id,
          targetId: user.id,
        },
